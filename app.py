...
def compute_sme_capability_score(turnover, years_active, employee_count, prior_contracts, certifications, contract_value):
    ...
    if turnover > 0 and contract_value > 0:
        ...
        if ratio >= 2.0:
            ...
            score += 15; breakdown.append(("Turnover vs Contract Value", 15, "Moderate", "Turnover meets contract value but below the 2x threshold preferred by buyers."))
        else:
    ...
    if years_active >= 5:
        score += 20; breakdown.append(("Years Active", 20, "Strong", str(years_active) + " years — established track record and organisational stability."))
    elif years_active >= 3:
        score += 12; breakdown.append(("Years Active", 12, "Moderate", str(years_active) + " years — some track record but buyers may prefer more established suppliers."))
    elif years_active >= 1:
        score += 6; breakdown.append(("Years Active", 6, "Weak", str(years_active) + " years — early stage. Many contracts require minimum 2-3 years trading history."))
    else:
        breakdown.append(("Years Active", 0, "Critical barrier", "Less than 1 year — most frameworks exclude very new businesses."))
    if employee_count >= 50:
        ...
        score += 7; breakdown.append(("Employee Count", 7, "Weak", str(employee_count) + " employees — limited capacity. May need to demonstrate subcontracting capability."))
    else:
    ...
    if prior_contracts >= 5:
        score += 20; breakdown.append(("Prior Public Contracts", 20, "Strong", str(prior_contracts) + " contracts won — strong procurement track record. Most valuable credential."))
    elif prior_contracts >= 2:
        score += 12; breakdown.append(("Prior Public Contracts", 12, "Moderate", str(prior_contracts) + " contracts — some experience. Buyers value this highly."))
    elif prior_contracts == 1:
        score += 6; breakdown.append(("Prior Public Contracts", 6, "Weak", "Only 1 prior contract — limited track record. Win smaller contracts to build history."))
    else:
        breakdown.append(("Prior Public Contracts", 0, "Critical barrier", "No prior contracts — most common SME barrier. Without history bids score lower on experience criteria."))
    if certifications >= 3:
        score += 15; breakdown.append(("Certifications", 15, "Strong", str(certifications) + " certifications — ISO, Cyber Essentials and sector certs strongly improve bid scores."))
    elif certifications >= 1:
        score += 8; breakdown.append(("Certifications", 8, "Moderate", str(certifications) + " certification(s) — additional certs would strengthen future bids."))
    else:
        breakdown.append(("Certifications", 0, "Weak", "No certifications — many contracts require ISO 9001 or Cyber Essentials minimum. Actively reduces bid scores."))
...
def explain_prediction(p,cv,cpv,region,rates):
    ...
    if cv<50000:
        reasons.append(("Value advantage","At £"+"{:,.0f}".format(cv)+", within the contract value range where SMEs are most competitive.","positive"))
    if not reasons:
        reasons.append(("Average conditions","Typical procurement characteristics. Win probability reflects the baseline SME award rate for this sector and region.","neutral"))
    return reasons
...
def analyse_sme_batch(sme_df,encoders,feature_cols,rates,scaler,rf,xgb,lr):
    ...
    for idx,sme in sme_df.iterrows():
        try:
            ...
            p_ens=rf.predict_proba(r)[0][1]*0.5+xgb.predict_proba(r)[0][1]*0.35+lr.predict_proba(r)[0][1]*0.15
            ...
            cap_barriers=", ".join([x[0] for x in breakdown if x[2] in ["Weak","Critical barrier"]]) or "None"
            results.append({
                "sme_name":sme.get("sme_name","SME_"+str(idx)),
                "sector":sme.get("sector",cpv_to_industry(cpv)),"region":reg,
                "turnover":turnover,"years_active":years,"employees":employees,
                "prior_contracts":prior,"certifications":certs,"contract_value":cv,
                "ml_win_probability":round(p_ens,3),
                "capability_score":cap_score,
                "combined_readiness_score":comb,
                "prediction":"Likely to win" if p_ens>=0.5 else "Unlikely to win",
                "market_barriers":barrier,"capability_barriers":cap_barriers,
                "recommendation":"Apply" if comb>=70 else "Prepare first" if comb>=40 else "Significant development needed",
            })
        except Exception: pass
...
def compute_sme_capability_score(turnover,years_active,employee_count,prior_contracts,certifications,contract_value):
    ...
    if employee_count>=50:
        score+=20; breakdown.append(("Employee Count",20,"Strong",str(employee_count)+" employees — sufficient capacity."))
    elif employee_count>=10:
        score+=13; breakdown.append(("Employee Count",13,"Moderate",str(employee_count)+" employees — adequate for smaller contracts."))
    elif employee_count>=3:
        ...
        breakdown.append(("Employee Count",2,"Critical barrier","Under 3 employees — most buyers question delivery capacity."))
    if prior_contracts>=5:
        score+=20; breakdown.append(("Prior Public Contracts",20,"Strong",str(prior_contracts)+" contracts — strong track record. Most valuable credential."))
    elif prior_contracts>=2:
...
with tab2:
    ...
    if st.button("Run barrier and gap analysis",type="primary",use_container_width=True,key="btn2"):
        ...
        if base_prob<0.3: st.error("Only "+"{:.1f}%".format(base_prob*100)+" probability — structural participation barrier confirmed.")
        elif base_prob<0.5: st.warning("{:.1f}%".format(base_prob*100)+" — below threshold. Bid costs outweigh expected return for most SMEs.")
        ...
        elif base_prob<0.5: st.warning("At 30-50%, SMEs face a rational choice — bid costs often outweigh expected returns.")
...
with tab3:
    ...
    if st.button("Fetch and score live contracts",type="primary",use_container_width=True):
        ...
        col4.metric("Above 50%",str((live_df["sme_win_probability"]>=0.5).sum())); col5.metric("Below 30%",str((live_df["sme_win_probability"]<0.3).sum()))
    ...
    with col_left:
        ...
        if cpv_input:
            ...
            if ir=="Unknown industry": st.warning("No industry found for: "+cpv_input)
                ...
                st.dataframe(pd.DataFrame({"CPV Code":related,"Industry":[ir]*len(related),"SME Rate":["{:.1f}%".format(rr2[c]*100) for c in related]}),use_container_width=True)
    with col_right:
        ...
        if industry_input:
            ...
            st.dataframe(pd.DataFrame({"CPV Code":cpv_list,"Historical SME Rate":["{:.1f}%".format(rs2[c]*100) for c in cpv_list]}),use_container_width=True)
            if rs2:
...
with tab6:
    ...
    st.markdown("Assess an SME using both **market-level ML prediction** and **organisational capability scoring** — combining contract accessibility with actual SME characteristics.")
    st.divider()
    ...
    if analysis_mode=="Single SME":
        ...
        st.caption("These characteristics determine the SME capability score — the true measure of procurement readiness.")
        col1,col2,col3=st.columns(3)
        with col1:
        ...
        if st.button("Assess SME procurement readiness",type="primary",use_container_width=True,key="sme_btn"):
            ...
            col1,col2,col3,col4=st.columns(4)
            col1.metric("ML Win Probability","{:.1f}%".format(p_s*100),help="Contract accessibility — based on sector, region, value")
            col2.metric("Capability Score",str(cap_score)+"/100",help="Organisational readiness — based on SME characteristics")
            col3.metric("Combined Readiness","{:.1f}%".format(comb),help="Overall procurement readiness combining both scores")
            col4.metric("Sector SME rate","{:.1f}%".format(cr_s*100))
            st.progress(comb/100)
            if comb>=70: st.success(sme_name+" is well-positioned for public procurement. Applying is recommended.")
            elif comb>=50: st.warning(sme_name+" has moderate readiness. Targeted improvements would increase competitiveness significantly.")
            elif comb>=30: st.error(sme_name+" has significant gaps. Focused development needed before bidding for this contract type.")
            else: st.error(sme_name+" is not yet ready. Fundamental barriers present — see recommendations below.")
            st.divider()
            ...
            for feat,pts,level,explanation in breakdown:
                if level=="Strong": st.success("**"+feat+" ("+str(pts)+"/"+str([25,20,20,20,15][[("Turnover vs Contract Value","Years Active","Employee Count","Prior Public Contracts","Certifications").index(feat) if feat in ("Turnover vs Contract Value","Years Active","Employee Count","Prior Public Contracts","Certifications") else 0][0])+"):** "+explanation)
        ...
        st.markdown("**Capability columns (optional but recommended):** turnover, years_active, employee_count, prior_contracts, certifications")
        ...
        if uploaded:
            ...
            if st.button("Analyse all SMEs",type="primary",use_container_width=True,key="batch_btn"):
                with st.spinner("Analysing SME profiles..."): batch_results=analyse_sme_batch(sme_input_df,encoders,feature_cols,rates,scaler,rf,xgb,lr)
                ...
                col3.metric("Avg combined readiness","{:.1f}%".format(batch_results["combined_readiness_score"].mean()))
                col4.metric("Ready to apply",str((batch_results["combined_readiness_score"]>=70).sum()))
...
with tab7:
    ...
    col2.metric("Regions below average","{:.0f}%".format(pct_below7))
    ...
    policy_text=(
        ...
        "5. Policy recommendation: Target highest-barrier sectors and regions with simplified frameworks.\n"
    )
