...
# ── LIVE CONTRACT FETCHING ────────────────────────────────────────────────────
@st.cache_data(ttl=3600)
def fetch_live_contracts(days_back=7, max_results=50):
    """
    Fetch live contracts from both UK procurement portals.
    Uses the TenderSearch OCDS API which proxies both CF and FAT
    and is publicly accessible without IP whitelisting.
    Falls back to direct API attempts if proxy available.
    """
    ...
    # ── SOURCE 1: Contracts Finder via OCDS bulk feed ────────────────────
    try:
        # Use the public OCDS release endpoint which does not require IP whitelist
    ...
    # ── SOURCE 2: Find a Tender OCDS API ─────────────────────────────────
    try:
        fat_url = "https://www.find-tender.service.gov.uk/api/1.0/ocds/notices/list"
        fat_r   = requests.get(
            fat_url,
            params={"publishedFrom": date_from, "publishedTo": date_to,
        ...
        if fat_r.status_code == 200:
            fat_data = fat_r.json()
            records  = fat_data.get("records", fat_data.get("releases", []))
            for rec in records:
    ...
    # ── FALLBACK: Generate realistic sample if both APIs blocked ─────────
    if not all_contracts:
        import random
        random.seed(int(datetime.now().timestamp()) % 1000)
        sectors = [
            ("IT services", "72000000"), ("Construction", "45000000"),
        ...
        buyers_cf = ["NHS Trust", "Local Council", "Ministry of Justice",
                     "Department for Education", "HMRC", "Home Office",
                     "Environment Agency", "Network Rail", "Transport for London"]
        buyers_fat = ["Cabinet Office", "MOD", "DVLA", "Companies House",
                      "Crown Commercial Service", "UKRI", "Innovate UK"]
        ...
        for i in range(min(max_results, 30)):
            sector_name, cpv = random.choice(sectors)
            ...
            all_contracts.append({
                "source":    source,
                "title":     f"{sector_name} Services Contract {2026}{i:03d}",
...
def gap_analysis(cv, cpv, region, encoders, feature_cols, rates, scaler, rf, xgb, lr):
    ...
    for test_cv, label in [
        (cv * 0.5,  f"Target 50% smaller contract (\u00a3{cv*0.5:,.0f})"),
        ...
        if p > base:
            sugg.append(("Contract size", label, round(p,3), round(p-base,3),
                          "Smaller contracts have higher SME award rates \u2014 build track record first"))
    best_reg = max(rates["region_sme_rate"], key=rates["region_sme_rate"].get)
    p_br = gp(cv, cpv, best_reg)
    if p_br > base:
        sugg.append(("Region", f"Target {best_reg}", round(p_br,3), round(p_br-base,3),
                      f"{best_reg} has historically higher SME award rates"))
    best_cpv = max(rates["cpv_sme_rate"], key=rates["cpv_sme_rate"].get)
    p_bc = gp(cv, best_cpv, region)
    if p_bc > base:
        sugg.append(("Sector", f"Consider {cpv_to_industry(best_cpv)}", round(p_bc,3), round(p_bc-base,3),
                      f"{cpv_to_industry(best_cpv)} sector has higher SME win rates historically"))
...
# ── PAGE ──────────────────────────────────────────────────────────────────────
st.set_page_config(page_title="SME Procurement Intelligence", page_icon="\U0001f3c6", layout="wide")
st.title("\U0001f3c6 SME Procurement Intelligence Platform")
...
if best_auc:
    st.success(f"\U0001f3c6 Best model: **{best_label}**  |  AUC-ROC: **{best_auc:.4f}**  \u2014 set as default")
st.divider()

tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "\U0001f52e Win Probability",
    "\U0001f4c8 Gap Analysis",
    "\U0001f4e1 Live Contracts",
    "\U0001f3ed CPV Lookup",
    "\U0001f4ca Historical",
])

# ── TAB 1: WIN PROBABILITY ────────────────────────────────────────────────────
with tab1:
    st.subheader("\U0001f52e Will this SME win the contract?")
    ...
    if st.button("\U0001f52e Predict win probability", type="primary", use_container_width=True, key="btn1"):
...
# ── TAB 2: GAP ANALYSIS ───────────────────────────────────────────────────────
with tab2:
    st.subheader("\U0001f4c8 What does this SME need to improve to win?")
    ...
    if st.button("\U0001f4ca Run gap analysis", type="primary", use_container_width=True, key="btn2"):
        ...
        st.divider()
        if suggestions:
            st.markdown("### \U0001f3af Steps to improve win probability:")
            for category, action, new_prob, improvement, advice in suggestions[:4]:
                with st.expander(f"\U0001f4a1 {action}  \u2192  {new_prob*100:.1f}%  (+{improvement*100:.1f}%)"):
        ...
        st.divider()
        st.markdown("### \U0001f4dd Why are SMEs not applying? \u2014 Model evidence")
...
with tab3:
    st.subheader("\U0001f4e1 Live UK Government Contracts")
    st.markdown("""
    Real-time contracts from **Contracts Finder** and **Find a Tender** \u2014 both sources running concurrently.
    Each contract is automatically scored for SME win probability. Data refreshes every hour.

    > **Note:** The UK government APIs restrict access by IP address. When live data is unavailable,
    > the platform generates a realistic representative sample based on current market patterns
    > so you can always test the prediction functionality.
    """)
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        filter_region = st.selectbox("Filter by region (optional)",
                                     ["All"] + list(encoders.get("region", {"Unknown": 0}).keys()))
        min_prob = st.slider("Minimum SME win probability", 0.0, 1.0, 0.0, 0.05)
    if st.button("\U0001f4e1 Fetch and score live contracts", type="primary", use_container_width=True):
        ...
        if len(live_df) == 0:
            st.error("No contracts returned. Please try again.")
        else:
            is_sample = not any(live_df.get("url", pd.Series()).str.startswith("http"))
            if is_sample:
                st.info("\U0001f4ca Live APIs are currently restricted. Showing a representative sample of UK procurement contracts scored by the model.")
            else:
                st.success(f"\u2705 Live data fetched successfully from UK government portals.")
            with st.spinner("Scoring all contracts for SME win probability..."):
                live_df["sme_win_probability"] = score_contracts(
                    live_df, encoders, feature_cols, rates, scaler, rf, xgb, lr)
            ...
            live_df["recommendation"] = live_df["sme_win_probability"].apply(
                lambda p: "\u2705 Apply \u2014 good chance" if p >= 0.6 else
                          "\U0001f914 Consider applying"     if p >= 0.4 else
            ...
            st.dataframe(
                live_df[["source","title","buyer","value","region","industry",
                          "sme_win_probability","recommendation","deadline"]]
                .rename(columns={"sme_win_probability": "SME Win %", "source": "Source"}),
                use_container_width=True
            )
            csv = live_df.to_csv(index=False)
            st.download_button("\U0001f4e5 Download as CSV", csv, "live_scored_contracts.csv", "text/csv")

# ── TAB 4: CPV LOOKUP ────────────────────────────────────────────────────────
with tab4:
    st.subheader("\U0001f3ed CPV Code \u2194 Industry Lookup")
        ...
        if cpv_input:
            ...
            if industry_result == "Unknown industry":
                ...
                df_related = pd.DataFrame({
                    "CPV Code": related,
                    "Industry": [industry_result] * len(related),
                    "SME Rate": [f"{related_rates[c]*100:.1f}%" for c in related],
                })
    ...
    with col_right:
        ...
        if industry_input:
            ...
            df_sector = pd.DataFrame({
                "CPV Code": cpv_list,
                "Historical SME Rate": [f"{rs[c]*100:.1f}%" for c in cpv_list],
            })
...
# ── TAB 5: HISTORICAL ─────────────────────────────────────────────────────────
with tab5:
    st.subheader("\U0001f4ca Historical SME Procurement Insights")
    ...
    try:
        ...
        rdf2["Recommended"] = rdf2["Model"].apply(
            lambda x: "\u2b50" if best_label in x or x in best_label else "")
...
# ── SIDEBAR ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.header("\U0001f3c6 SME Procurement Intelligence")
    st.markdown("**What this platform does:**")
    st.markdown("\U0001f52e Predicts if an SME will win a contract")
    st.markdown("\U0001f4c8 Identifies what to improve to win")
    st.markdown("\U0001f4e1 Shows live contracts from both UK portals")
    st.markdown("\U0001f3ed Maps CPV codes to industries")
    st.markdown("\U0001f4ca Reveals historical SME award patterns")
    st.divider()
    if best_auc:
